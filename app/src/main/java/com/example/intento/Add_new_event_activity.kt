...
//Botones para los días
    ...
    private fun saveNewEvent(allDaySwitch: SwitchCompat,l_presionado:Boolean,m_presionado:Boolean,mie_presionado:Boolean,j_presionado:Boolean,v_presionado:Boolean,sa_presionado:Boolean,do_presionado:Boolean) {
        ...
        if (to_hora.length<from_hora.length){
            Toast.makeText(this, "Insert a correct time frame", Toast.LENGTH_LONG).show()
            from_hora = "00:00"
            to_hora="00:00"
        }else if(to_hora.length==from_hora.length){
        if (to_hora.length==4){
            to_hora = "0"+to_hora
        ...
        dbRef_calendars.child(calendario_ingresado).child("events").child(eventId).setValue("true")
...
//-----------------------------------------------------------------------------------------------------------------------------
    ...
    save_new_event.setOnClickListener {
        ...
        val intent_upcoming = Intent(this, Event_main_activity::class.java)
