...
class ConsultarUserActivity : AppCompatActivity() {
    ...
    private lateinit var LinearLayoutPassword:TextView
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val regresar_profile_info: ImageButton = findViewById(R.id.regresar_profile_info)
        regresar_profile_info.setOnClickListener{
            val intent_account = Intent(this, Activity_drawer_menu::class.java)
        ...
        dbRef_nuevousuario.child(idUsername).addListenerForSingleValueEvent(object : ValueEventListener {
            override fun onDataChange(dataSnapshot: DataSnapshot) {
                if (dataSnapshot.exists()) {
                    ...
                    LinearLayoutPassword.text = password
                } else {
    ...
    private fun user_sign_out() {
        ...
        dbRefCurrentUser.removeValue().addOnCompleteListener { task ->
            if (task.isSuccessful) {
                ...
                // Handle error
                Toast.makeText(this@ConsultarUserActivity, "Error al cerrar sesión: ${task.exception?.message}", Toast.LENGTH_SHORT).show()
            }
        }
    }
}
